...
relevant here\footnote{Here, `recently' refers to when I first wrote the first version of these notes, the work mentioned eventually appeared as \texttt{doi.org/10.1371/journal.pcsy.0000030} where these data, or something like them, appear as Figure 8.}, I measured the relationship between two variables, here
...
\begin{equation}
  \mathcal{E}_p=\sqrt[p]{\frac{1}{n}\sum_i (\hat{y}_i-y)^p}
\end{equation}
...
perform to be differentiated. 

As an aside this is the essence of \texttt{autograd}, the automatic
differentiation routines that make modern machine learning possible:
automatic differentiation relies on the simple observation that, in
general, if a calculation is expressed as a series of smaller
calculation, one on top of the other, then the chain rule means that the derivative of the overall calculation can be expressed in terms of the individual steps: in short, in some vague way, if it can be calculated, if can be differenciated.
...
different people use different systems of notation, but also because
...
Of course, when we move to more dimesions we immeadiately start to see
the limitation of picking a regression model; for two dimensions for example
\begin{equation}
y=\beta_0+\beta_1x_1+\beta_2x_2
\end{equation}
we see that according to the model the way $y$ changes with $x_2$ for
example, doesn't depending on $x_1$: to make that explicit, if we add
one to $x_2$ that adds $\beta_2$ to $y$, no matter what the value of
$x_1$ is. It is easy to imagine data where that would be wrong, where,
for example,
\begin{equation}
y=\beta_0+\beta_1x_1+\beta_2x_2+\beta_{12}x_1x_2
\end{equation}
would be a better model; in statistics this is called an
\emph{interaction term} and interactions like this are often of huge
interest in psychology, sociology and economics where great effort is
often put in deciding whether an interaction term is `statistically
significant'.  The main point, for now, is that the linear model
$y=mx+c$ is just a starting point and often a surprisingly good one,
but in practice, in `classical data analysis' we often want to
consider a broader class of models while avoiding the danger of just
wandering in model space or finding ourselves with more model
parameters than our data can hope to pin down. As we will see, neural
networks can be considered as a way to parameterize models and
optimize them. It is interesting to consider how our mythical
classical data analysis, with its concern for parameter interpretation
and statistical significance, relates to neural networks. However, we
will first ask how, in general, we can fit more complex models, that
is, how we find the $\beta$s, this often, even usually, involves gradient flow.

As one final note before going on to gradient flow, regression models often need to be
